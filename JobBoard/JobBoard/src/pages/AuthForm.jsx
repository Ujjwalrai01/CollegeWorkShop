...
//       // Simulate success
//       alert(
//         `${isSignUp ? "Registration" : "Login"} successful for ${userType}!`
//       );
//     } catch (error) {
...
//             <div
//               className={`inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br ${currentUserType.color} mb-4`}
//             >
...
//                 : `Welcome back, ${currentUserType.label}`}
//             </h2>
...
//               } hover:opacity-90 text-white font-semibold rounded-xl shadow-lg transition-all duration-200 flex items-center justify-center space-x-2 ${
//                 isLoading
...
//               "Unlock opportunities with the most intelligent job-matching platform. Built for the modern job seeker."}
//             {userType === "recruiter" &&
//               "Connect with qualified candidates and streamline your hiring process with our advanced recruiting tools."}
//             {userType === "admin" &&
//               "Comprehensive platform management tools to oversee users, content, and system operations."}
//           </p>
//           <ul className="space-y-3 text-sm text-left text-purple-200 max-w-md">
//             {userType === "seeker" && (
//               <>
...
const EnhancedAuthForm = () => {
  ...
  const userTypes = [
    {
      ...
      description: "Find your dream job",
      color: "from-blue-500 to-purple-500",
      route: "/dashboard/seeker", // Add route for each user type
    },
    {
      id: "recruiter",
      label: "Recruiter",
  ...
  // Navigation function - you can replace this with your routing logic
  const navigateToUserDashboard = (userType) => {
    const userTypeData = userTypes.find((type) => type.id === userType);

    // Option 1: Using React Router (if you have react-router-dom installed)
    const navigate = useNavigate();
    navigate(userTypeData.route);

    // Option 2: Using window.location (simple redirect)
    // window.location.href = userTypeData.route;

    // Option 3: Using history API (for SPA navigation)
    // window.history.pushState({}, '', userTypeData.route);

    // Option 4: Custom navigation handler (you can pass this as a prop)
    // if (onNavigate) {
    //   onNavigate(userTypeData.route, userType);
    // }

    // For demo purposes, we'll just show an alert and log the navigation
    console.log(
      `Navigating to: ${userTypeData.route} for user type: ${userType}`
    );
    alert(`Success! Redirecting to ${userTypeData.label} dashboard...`);

    // You can also store user info in localStorage/sessionStorage before navigation
    // localStorage.setItem('user', JSON.stringify({ userType, ...formData }));
  };
